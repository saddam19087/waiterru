...
export default function SectionTitleLineWithButton({ icon,cliclkFun, title, main = false, children }: Props) {
  ...
  return (
    <section className={`${main ? '' : 'pt-6'} mb-6 flex items-center justify-between`}>
      ...
      {children}
      {!hasChildren && <Button onClick={cliclkFun} icon={mdiPlus} label='ADD MENU' color="whiteDark" />}
    </section>
  )
}
